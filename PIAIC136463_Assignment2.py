...
def function8():
    ...
    x = np.zeros(10,dtype=np.uint32).reshape(2,5); x.fill(6) #write your code here
...
def function12():
    ...
    arr = np.array([1,2,3])
    ans = np.concatenate([np.stack((arr,arr,arr),axis=1).reshape(-1), np.stack((arr,arr,arr),axis=0).reshape(-1)]) #write your code here  #write your code here 
...
def function18():
    ...
    x = np.count_nonzero(piaic == students) # Write you code Here
    return x
